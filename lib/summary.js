...
class Summary extends Metric {
	constructor(config) {
		...
		if (this.labelNames.length === 0) {
			this.store.getOrAdd({}, storedLabels =>
				createBaseValues(storedLabels, this.maxAgeSeconds, this.ageBuckets),
			);
		}
	}
	...
	zero(labels) {
		...
		this.store.merge(labels, {
			value: createBaseValues(labels, this.maxAgeSeconds, this.ageBuckets),
		});
	}
...
function createBaseValues(labels, maxAgeSeconds, ageBuckets) {
	return {
		labels,
		td: new timeWindowQuantiles(maxAgeSeconds, ageBuckets),
		count: 0,
		sum: 0,
	};
}
